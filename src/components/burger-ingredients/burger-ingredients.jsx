  ...
  return (
    <section className={burgerIngredients.section}>
      ...
      <div className={`${burgerIngredients.tabWrapper} mb-10`}>
        <Tab value="bun" active={tab === 'bun'} onClick={() => handleNavigationClick('bun')}>
          Булки
        </Tab>
        <Tab value="sauce" active={tab === 'sauce'} onClick={() => handleNavigationClick('sauce')}>
          Соусы
        </Tab>
        <Tab value="main" active={tab === 'main'} onClick={() => handleNavigationClick('main')}>
          Начинки
        </Tab>
      </div>
      <div className={`${burgerIngredients.ingredientsWrapper} custom-scroll`}>
        <IntersectionObserver id='bun' onChange={handleIntersection} threshold={.9}>
          <div ref={bunRef}>
            <h2 className={`${burgerIngredients.heading} mb-6 text text_type_main-medium`}>Булки</h2>
        ...
        <IntersectionObserver id='sauce' onChange={handleIntersection} threshold={.8}>
          <div ref={sauceRef}>
            <h2  className={`${burgerIngredients.heading} mt-2 mb-5 text text_type_main-medium`}>Соусы</h2>
            <ul className={burgerIngredients.ingredientsContainer}>
              {sauces.map((ingredient) => <BurgerIngredient key={ingredient._id} ingredient={ingredient} />)}
            </ul>
          </div>
        </IntersectionObserver>
        <IntersectionObserver id='main' onChange={handleIntersection} threshold={.35}>
          <div ref={mainRef}>
